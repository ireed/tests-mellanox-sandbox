 ...
 ***************************************************************************/
int osh_sync_tc1(const TE_NODE *node, int argc, const char *argv[])
{
    int rc = TC_PASS;
...
static int test_item2(void)
{
    ...
    {
        rc = TC_SETUP_FAIL;
        log_debug(OSH_TC, "test need at least 2 PEs\n");
        return rc;
    }
    ...
    (*shmem_indx) = 0;
    shmem_barrier_all(); // Make sure that initial index value is visible to all PEs
    ...
    if (my_proc == num_proc - 1)
    {
        for (i = 0; i < (num_proc - 1) * CYCLE_COUNT; i++)
        {
            ...
            {
                rc = TC_FAIL;
                log_error(OSH_TC, "bad data: i=%d val=%d\n", i, shmem_array[i]);
            }
        }
    }
