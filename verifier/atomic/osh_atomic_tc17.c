 ...
 ***************************************************************************/
int osh_atomic_tc17(const TE_NODE *node, int argc, const char *argv[])
{
    int rc = TC_PASS;
...
static int test_item1(void)
{
    ...
    if (shmem_addr)
    {
        ...
        *shmem_addr = 0;
        ...
        expect_value = ( my_proc == root_proc ? ((num_proc - 1) * num_proc / 2) * __cycle_count : 0);

        shmem_barrier_all();
        for (i = 0; (i < __cycle_count) && (rc == TC_PASS); i++)
        {
            FUNC_VALUE(shmem_addr, my_value, root_proc);
        }
        shmem_barrier_all();
...
static int test_item2(void)
{
    ...
    if (shmem_addr)
    {
        ...
        *shmem_addr = 0;
        ...
        for (i = 0; (i < __cycle_count) && (rc == TC_PASS); i++)
        {
            FUNC_VALUE(shmem_addr, my_value, peer_proc);
        }
        shmem_barrier_all();
