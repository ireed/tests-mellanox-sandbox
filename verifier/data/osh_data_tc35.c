...
#define FUNC_VALUE  shmem_get128
 ...
 ***************************************************************************/
int osh_data_tc35(const TE_NODE *node, int argc, const char *argv[])
{
    int rc = TC_PASS;
...
static int test_item1(void)
{
    ...
    if (shmem_addr)
    {
        ...
        /* Set my value */
        my_value.field1 = (((double)rand() / (double)RAND_MAX) * MAX_VALUE);
        memcpy(shmem_addr, &my_value, sizeof(my_value));

        /* Define peer and it value */
        peer_proc = my_proc;
        memcpy(&peer_value, &my_value, sizeof(my_value));
...
static int test_item3(void)
{
    ...
    if (shmem_addr && recv_addr)
    {
        ...
        {
            /* Set my value */
            my_value.field1 = (my_proc % 2 ? 1 : -1) * (i * (MAX_VALUE / __cycle_count));
