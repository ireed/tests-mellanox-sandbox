...
#define FUNC_VALUE  shmem_longdouble_get
 ...
 ***************************************************************************/
int osh_data_tc22(const TE_NODE *node, int argc, const char *argv[])
{
    int rc = TC_PASS;
...
static int test_item3(void)
{
    ...
    if (shmem_addr && recv_addr)
    {
        ...
        for (i = 0; (i < __cycle_count) && (rc == TC_PASS); i++)
        {
            ...
            rc = (!compare_buffer_with_const_longdouble(recv_addr, cur_buf_size, expect_value) ? TC_PASS : TC_FAIL);
