...
#define FUNC_VALUE  shmem_getmem
 ...
 ***************************************************************************/
int osh_data_tc31(const TE_NODE *node, int argc, const char *argv[])
{
    int rc = TC_PASS;

    rc = __parse_opt(node, argc, argv);

    /* Get random the single value as symmetric data object from itself */
    if (rc == TC_PASS)
    {
        rc = test_item1();
        log_item(node, 1, rc);
...
#if 0 /* Spec: UNDEFINED BEHAVIOR */
    /* Get random the single value as usual local variable from itself */
    if (rc == TC_PASS)
    {
        rc = test_item2();
        log_item(node, 2, rc);
    ...
    /* Get buffer as symmetric allocated from itself */
    if (rc == TC_PASS)
    {
        rc = test_item4();
        log_item(node, 4, rc);
...
#if 0 /* Spec: UNDEFINED BEHAVIOR */
    /* Get buffer as local from itself */
    if (rc == TC_PASS)
    {
        rc = test_item5();
        log_item(node, 5, rc);
...
static int test_item5(void)
{
    ...
    TYPE_VALUE recv_addr[MAX_BUFFER_SIZE];
    TYPE_VALUE my_value = 0;
    TYPE_VALUE peer_value = 0;
    TYPE_VALUE expect_value = 0;
    int num_proc = 0;
