...
#define FUNC_VALUE  shmem_put128
#define SIZE_VALUE  sizeof(TYPE_VALUE)
#define MAX_VALUE   (((unsigned long long)1 << ( 8 * sizeof(uint32_t) - 1)) - 1)
 ...
 ***************************************************************************/
int osh_data_tc38(const TE_NODE *node, int argc, const char *argv[])
{
    int rc = TC_PASS;

    rc = __parse_opt(node, argc, argv);

    /* Put random the single value to itself */
    if (rc == TC_PASS)
    {
        rc = test_item1();
        log_item(node, 1, rc);
...
static int test_item1(void)
{
    ...
    if (shmem_addr)
    {
        ...
        peer_proc = my_proc;
        peer_value.field1 = (((double)rand() / (double)RAND_MAX) * MAX_VALUE);
...
static int test_item2(void)
{
    ...
    if (shmem_addr)
    {
        ...
        peer_value.field1 = my_proc;

        /* Define expected value */
        expect_value.field1 = (my_proc ? (my_proc - 1) : (num_proc - 1));
...
static int test_item3(void)
{
    ...
    if (shmem_addr && send_addr)
    {
        ...
        for (i = 0; (i < __cycle_count) && (rc == TC_PASS); i++)
        {
            ...
            /* Define peer and it value */
            peer_proc = my_proc;
            peer_value.field1 = (peer_proc % 2 ? 1 : -1) * (i * (MAX_VALUE / __cycle_count));
            ...
            log_debug(OSH_TC, "my(#%d:%lld.%lld) peer(#%d:%lld.%lld) expected = %lld.%lld buffer size = %lld\n",
                my_proc, (INT64_TYPE)my_value.field1, (INT64_TYPE)my_value.field2,
                peer_proc, (INT64_TYPE)peer_value.field1, (INT64_TYPE)peer_value.field2,
                (INT64_TYPE)expect_value.field1, (INT64_TYPE)expect_value.field2,
                (INT64_TYPE)cur_buf_size);
