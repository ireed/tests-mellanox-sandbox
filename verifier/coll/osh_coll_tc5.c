 ...
 ***************************************************************************/
int osh_coll_tc5(const TE_NODE *node, int argc, const char *argv[])
{
    int rc = TC_PASS;
    ...
    /* All PEs gather the single value from peers */
    if (rc == TC_PASS)
    {
        rc = test_item2();
        log_item(node, 2, rc);
    ...
    /* Every PE put the buffer as symmetric data object to itself */
    if (rc == TC_PASS)
    {
        rc = test_item3();
        log_item(node, 3, rc);
    ...
    /* All PEs gather the dynamic buffer from peers */
    if (rc == TC_PASS)
    {
        rc = test_item4();
        log_item(node, 4, rc);
    ...
    /* All PEs gather the static buffer from peers */
    if (rc == TC_PASS)
    {
        rc = test_item5();
        log_item(node, 5, rc);
    ...
    /* collect calls in loop with alternating multiple pSync arrays (without barrrier synchronization between iterations) */
    if (rc == TC_PASS)
    {
        rc = test_item6();
        log_item(node, 6, rc);
    ...
    /* Only root PE gather data, other has zero data length */
	if (rc == TC_PASS)
    {
        rc = test_item7();
        log_item(node, 7, rc);
...
static int test_item1(void)
{
    ...
    send_addr = shmem_malloc(sizeof(*send_addr));
    if (shmem_addr && send_addr)
    {
        ...
        *send_addr = peer_value;
        ...
        /* Put value to peer */
        FUNC_VALUE(shmem_addr, send_addr, 1, my_proc, 0, 1, pSync);
...
static int test_item2(void)
{
    ...
    cur_buf_size = num_proc;
    shmem_addr = shmem_malloc(sizeof(*shmem_addr) * cur_buf_size);
    send_addr = shmem_malloc(sizeof(*send_addr));
    if (shmem_addr && send_addr)
    {
        TYPE_VALUE value = DEFAULT_VALUE;
        ...
        *send_addr = peer_value;
        ...
        /* Put value to peer */
        FUNC_VALUE(shmem_addr, send_addr, 1, 0, 0, num_proc, pSync);
        ...
        log_debug(OSH_TC, "my#%d root(#%d:%lld) expected = %lld actual = %lld group size = %lld\n",
...
static int test_item3(void)
{
    ...
    send_addr = (TYPE_VALUE*)shmem_malloc(sizeof(*send_addr) * __max_buffer_size);
    if (shmem_addr && send_addr)
    {
        TYPE_VALUE value = DEFAULT_VALUE;
        int i = 0;
        ...
        for (i = 0; (i < __cycle_count) && (rc == TC_PASS); i++)
        {
            ...
            /* Put value to peer */
            FUNC_VALUE(shmem_addr, send_addr, cur_buf_size, my_proc, 0, 1, pSync);
...
static int test_item4(void)
{
    ...
    shmem_addr = (TYPE_VALUE*)shmem_malloc(sizeof(*shmem_addr) * __max_buffer_size * num_proc);
    send_addr = (TYPE_VALUE*)shmem_malloc(sizeof(*send_addr) * __max_buffer_size);
    if (shmem_addr && send_addr)
    {
        TYPE_VALUE value = DEFAULT_VALUE;
        int i = 0;
        int j = 0;
        long recv_buf_size = 0;
        long send_buf_size = 0;

        for (i = 0; (i < __cycle_count) && (rc == TC_PASS); i++)
        {
            send_buf_size = sys_max(1, (i + 1) * __max_buffer_size / __cycle_count);
            recv_buf_size = send_buf_size * num_proc;

            /* Set my value */
            my_value = DEFAULT_VALUE;
            fill_buffer((void *)shmem_addr, recv_buf_size, (void *)&my_value, sizeof(my_value));
            ...
            /* Define peer and it value */
            peer_value = (i * (MAX_VALUE / __cycle_count));
            fill_buffer((void *)send_addr, send_buf_size, (void *)&peer_value, sizeof(peer_value));
            ...
            /* Put value to peer */
            FUNC_VALUE(shmem_addr, send_addr, send_buf_size, 0, 0, num_proc, pSync);
            ...
            rc = (!compare_buffer_with_const(shmem_addr, recv_buf_size, &expect_value, sizeof(expect_value)) ? TC_PASS : TC_FAIL);

            log_debug(OSH_TC, "my#%d root(#%d:%lld) expected = %lld actual = %lld buffer size = %lld\n",
                               my_proc, root_proc, (INT64_TYPE)peer_value, (INT64_TYPE)expect_value, (INT64_TYPE)value, (INT64_TYPE)send_buf_size);
...
static int test_item5(void)
{
...

    {
        ...
        for (i = 0; (i < __cycle_count) && (rc == TC_PASS); i++)
        {
            send_buf_size = sys_max(1, (i + 1) * (MAX_BUFFER_SIZE / num_proc) / __cycle_count);
            recv_buf_size = send_buf_size * num_proc;
...
static int test_item6(void)
{
    ...
    if (rc == TC_PASS)
    {
        ...
        send_buf_size = MAX_BUFFER_SIZE / num_proc;
        recv_buf_size = send_buf_size * num_proc;
        ...
        for (i = 0; (i < __cycle_count) && (rc == TC_PASS); i++)
        {
            ...
            if (rc)
            {
                ...
                fill_buffer((void *)(send_addr + (i % 2) * MAX_BUFFER_SIZE), send_buf_size, (void *)&peer_value, sizeof(peer_value));
                fill_buffer((void *)(shmem_addr + (i % 2) * MAX_BUFFER_SIZE ), recv_buf_size, (void *)&my_value, sizeof(my_value));
        }
    }
...
static int test_item7(void)
{
	...
	root_proc = (num_proc > 2 ? 1 : 0);

    shmem_addr = shmem_malloc(sizeof(*shmem_addr) * num_proc);
    send_addr = shmem_malloc(sizeof(*send_addr));
	...
	if (shmem_addr && send_addr)
    {
		...
		/* Define peer and it value */
        *send_addr = BASE_VALUE + my_proc;

		fill_buffer((void *)shmem_addr, num_proc, (void *)&my_value, sizeof(my_value));
        ...
        /* Put value to peer */
        FUNC_VALUE(shmem_addr, send_addr, (my_proc == root_proc ? 1 : 0), 0, 0, num_proc, pSync);
		...
		rc = TC_PASS;
       	...
       	{
           	if ( (j == 0 && shmem_addr[j] != (BASE_VALUE + root_proc)) ||
				 (j != 0 && shmem_addr[j] != DEFAULT_VALUE) )
			{
				rc = TC_FAIL;
				break;
			}
