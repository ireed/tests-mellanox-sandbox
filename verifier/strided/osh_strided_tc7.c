 ...
 ***************************************************************************/
int osh_strided_tc7(const TE_NODE *node, int argc, const char *argv[])
{
    int rc = TC_PASS;
...
static int test_item1(void)
{
    ...
    if (shmem_addr)
    {
        ...
        for (i = 0; (i < COUNT_VALUE) && (rc == TC_PASS); i++)
        {
            ...
            if (rc == TC_PASS)
            {
                 rc = (compare_longdouble_buffers(local_addr,expect_value, MAX_ARRAY_SIZE, &odd_pos) ? TC_PASS : TC_FAIL);
            }
