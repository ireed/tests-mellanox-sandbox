 ...
 ***************************************************************************/
int osh_strided_tc10(const TE_NODE *node, int argc, const char *argv[])
{
    int rc = TC_PASS;
...
static int test_item1(void)
{
    ...
    if (shmem_addr)
    {
        ...
        for (i = 0; (i < COUNT_VALUE) && (rc == TC_PASS); i++)
        {
            ...
            {
                shmem_addr[j].field1 = my_value;
                shmem_addr[j].field2 = my_value;
            }


            /* Define peer and it value */
            peer_proc = (my_proc + 1) % num_proc;
            peer_value = (peer_proc + 1);
            ...
            if (rc == TC_PASS)
            {
                rc = (!memcmp(local_addr,expect_value,MAX_ARRAY_SIZE*SIZE_VALUE)) ? TC_PASS : TC_FAIL;
            }
            log_debug(OSH_TC, "my(#%d:%lld) peer(#%d:%lld) expected = %lld vs got = %lld\n",
                               my_proc, (INT64_TYPE)my_value, peer_proc, (INT64_TYPE)peer_value, (INT64_TYPE)expect_value[0].field1, (INT64_TYPE)local_addr[0].field1);
