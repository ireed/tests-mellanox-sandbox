 ...
 ***************************************************************************/
int osh_strided_tc11(const TE_NODE *node, int argc, const char *argv[])
{
    int rc = TC_PASS;
    ...
    if (shmem_addr)
    {
        ...
        for (i = 0; (i < COUNT_VALUE) && (rc == TC_PASS); i++)
        {
            ...
            log_debug(OSH_TC, "my(#%d:%lld) peer(#%d:%lld) expected = %lld vs got = %lld\n",
                               my_proc, (INT64_TYPE)my_value, peer_proc, (INT64_TYPE)peer_value, (INT64_TYPE)expect_value[0], (INT64_TYPE)shmem_addr[0]);
