...
#define OVERFLOW_FACTORIAL_LIMIT (10) /* (10 is for double)
 ...
 ***************************************************************************/
int osh_reduce_tc44(const TE_NODE *node, int argc, const char *argv[])
{
    int rc = TC_PASS;
...
static int test_item1(void)
{
    ...
    if (target_addr && source_addr)
    {
        ...
        *source_addr = source_value;
        ...
        {
            int total_wait = 0;
            while (sys_ccompare(*target_addr, DEFAULT_VALUE) && total_wait < 1000 * WAIT_COUNT)
            {
                total_wait++;
                usleep(1);
            }
        ...
        rc = (sys_ccompare(expect_value, value) ? TC_PASS : TC_FAIL);
        ...
        for (i = 0; (i < __cycle_count) && (rc == TC_PASS); i++)
        {
            ...
            if (pWrk)
            {
                ...
                shmem_barrier_all();
                {
                    ...
                    {
                        value = *target_addr;
                        if (sys_ccompare(expect_value, value)) break;
                        sleep(1);
                    }
                }
