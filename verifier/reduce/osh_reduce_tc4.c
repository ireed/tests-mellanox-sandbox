 ...
 ***************************************************************************/
int osh_reduce_tc4(const TE_NODE *node, int argc, const char *argv[])
{
    int rc = TC_PASS;
...
static int test_item2(void)
{
    ...
    if (target_addr && source_addr)
    {
        ...
        *target_addr = DEFAULT_VALUE;
        ...
        /* Set my value */
        source_value = (TYPE_VALUE)( my_proc % 2 ? BASE_VALUE : num_proc );
        *source_addr = source_value;

        /* Define expected value */
        expect_value = (TYPE_VALUE)((TYPE_VALUE)BASE_VALUE & (TYPE_VALUE)num_proc);
    ...
    if (target_addr && source_addr)
    {
        ...
        for (i = 0; (i < __cycle_count) && (rc == TC_PASS); i++)
        {
            ...
            if (pWrk)
            {
                ...
                /* Set my value */
                source_value = ( my_proc % 2 ? my_proc : (my_proc == 0 ? BASE_VALUE : num_proc ));
                fill_buffer((void *)source_addr, cur_buf_size, (void *)&source_value, sizeof(source_value));
...
static int test_item9(void)
{
    ...
    if (pSyncMult)
    {
        ...
        {
            value = DEFAULT_VALUE;
            source_value = (TYPE_VALUE)( my_proc % 2 ? my_proc : (my_proc == 0 ? BASE_VALUE : num_proc ));
