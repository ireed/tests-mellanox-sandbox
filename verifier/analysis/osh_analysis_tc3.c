 ...
 ***************************************************************************/
int osh_analysis_tc3(const TE_NODE *node, int argc, const char *argv[])
{
    int rc = TC_PASS;
        ...
        if (shmem_addr && send_addr)
        {
            ...
            for (nelems = 1; (nelems <= (__report_msize / (long)SIZE_VALUE)) && (rc == TC_PASS); nelems<<=1)
            {
                ...
                while (i--)
                {
                    ...
                    {
                        sys_memcpy((void*)shmem_addr, (void*)send_addr, (nelems * SIZE_VALUE));
                    }
                    FUNC_VALUE(shmem_addr, send_addr, nelems, root_proc, 0, 0, num_proc, pSync);
                    shmem_barrier_all();
                }
                ...
                rc = (rc ? rc : (shmem_addr[0] == root_proc ? TC_PASS : TC_FAIL));
